...
#    generator = BatchGenerator(sys.argv[1])
    ...
    x_t_n0 = Input(shape=(1,), dtype='float32', name='month/x_t_n0')
    x_c_n0 = Input(shape=(1,), dtype='float32', name='x_c_n0')
    ...
    x = Flatten()(x)
    x = concatenate([x, x_c_c, embedding_2_d_1, embedding_2_d_2, x_t_n0, x_c_n0])
    x = Dense(256, activation='relu')(x)
    x = Dense(64, activation='relu')(x)
    x = concatenate([x, x_wide])
    output = Dense(1, activation='sigmoid')(x)

    model= Model(inputs=[x_t_c, x_t_d, x_c_c, x_c_d_1, x_c_d_2, x_wide, x_t_n0, x_c_n0], outputs=output)
