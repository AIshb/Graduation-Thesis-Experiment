...
def identity_block(input_tensor, kernel_size, filters, time, stage, block):
    nb_filter1, nb_filter2, nb_filter3 = filters
    conv_name_base = time + '/' + 'res' + str(stage) + block + '_branch'
    bn_name_base = time + '/' + 'bn' + str(stage) + block + '_branch'
...
def conv_block(input_tensor, kernel_size, filters, time, stage, block, stride=2):
    nb_filter1, nb_filter2, nb_filter3 = filters
    conv_name_base = time + '/' + 'res' + str(stage) + block + '_branch'
    bn_name_base = time + '/' + 'bn' + str(stage) + block + '_branch'
...
def build_model(*args, **kwargs):
    ...
    x = conv_block(x, 3, [64, 64, 256], time='month', stage=2, block='a', stride=1)
    x = identity_block(x, 3, [64, 64, 256], time='month', stage=2, block='b')

    x = conv_block(x, 3, [128, 128, 512], time='month', stage=3, block='a')
    x = identity_block(x, 3, [128, 128, 512], time='month', stage=3, block='b')
    x = identity_block(x, 3, [128, 128, 512], time='month', stage=3, block='c')

#    month_x = Flatten()(x)
    month_x = GlobalMaxPooling1D()(x)
    ...
    x = conv_block(x, 3, [64, 64, 256], time='day', stage=2, block='a', stride=1)
    x = identity_block(x, 3, [64, 64, 256], time='day', stage=2, block='b')

    x = conv_block(x, 3, [128, 128, 512], time='day', stage=3, block='a')
    x = identity_block(x, 3, [128, 128, 512], time='day', stage=3, block='b')
    x = identity_block(x, 3, [128, 128, 512], time='day', stage=3, block='c')

#    day_x = Flatten()(x)
    day_x = GlobalMaxPooling1D()(x)
